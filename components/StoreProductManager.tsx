...
const ERROR_BORDER = { borderColor: '#F2CFCB' }

const PILL_GREEN = { backgroundColor: 'rgba(31,122,85,0.08)', color: '#1F7A55' }
const PILL_AMBER = { backgroundColor: 'rgba(138,90,22,0.10)', color: '#8A5A16' }
const PILL_RED = { backgroundColor: 'rgba(153,27,27,0.08)', color: '#B3382B' }
const PILL_GREY = { backgroundColor: 'rgba(22,22,26,0.06)', color: '#8A8A92' }
  ...
  return (
    ...
    >
      <div
        ...
        style={{ backgroundColor: value ? '#16161A' : 'rgba(22,22,26,0.12)' }}
      >
        <div
...
export default function StoreProductManager({ storeId, clientSlug, currency }: Props) {
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0 space-y-1">
          ...
          <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
            <span className="yele-pill" style={item.active ? PILL_GREEN : PILL_GREY}>
              {item.active ? 'Visible' : 'Hidden'}
            </span>
            {item.track_inventory && (
              <span className="yele-pill" style={stockStyle}>
                {stock === 0 ? 'Sold out' : `${stock} in stock`}
              </span>
            )}
            {item.featured && (
              <span className="yele-pill" style={{ backgroundColor: 'rgba(212,111,200,0.12)', color: '#8A5A16' }}>
                Featured
              </span>
            )}
          </div>
  ...
  return (
    <div className="yele-card overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          <Package size={14} style={{ color: '#D46FC8' }} />
          <h3 className="font-semibold text-sm" style={{ color: '#16161A', fontFamily: 'var(--font-display)' }}>
            Products
          </h3>
          <span
        ...
        <button onClick={openAdd} className="yele-btn yele-btn-primary">
          <Plus size={14} />
          Add product
        </button>
      </div>
      ...
      <div className="p-4">
        {loading ? (
          <div className="flex flex-col gap-2 animate-pulse">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-20 rounded-xl" style={{ backgroundColor: '#F2F0EB' }} />
            ))}
          </div>
        ) : items.length === 0 ? (
          <div className="text-center py-10">
            ...
            <button
              onClick={openAdd}
              className="yele-btn yele-btn-ghost mt-3"
              style={{ color: '#D46FC8' }}
            >
                    ...
                    <div className="yele-card-quiet p-4 relative group">
                      ...
                      <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          style={{ padding: '6px', backgroundColor: 'rgba(22,22,26,0.06)' }}
                          onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'rgba(22,22,26,0.12)' }}
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 p-5 space-y-4 overflow-y-auto">
                {/* Name */}
                <div>
                  <label className="yele-label">Name<span style={{ color: '#D46FC8' }}> *</span></label>
                  <input
                    className="yele-input"
                    style={errors.name ? ERROR_BORDER : undefined}
                    value={form.name}
                    onChange={(e) => patch({ name: e.target.value })}
                    placeholder="e.g. Business Website"
                  />
                  {errors.name && (
                    <p className="text-xs mt-1" style={{ color: '#B3382B', fontFamily: 'var(--font-instrument)' }}>
                      {errors.name}
                    </p>
                  )}
                </div>
                ...
                {/* Category */}
                <div>
                  <label className="yele-label">Category</label>
                  <input
                    className="yele-input"
                    value={form.category}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="yele-label">Price<span style={{ color: '#D46FC8' }}> *</span></label>
                    <input
                      type="number"
                      step="0.01"
                      min="0"
                      className="yele-input"
                      style={errors.price ? ERROR_BORDER : undefined}
                      value={form.price}
                  ...
                  <div>
                    <label className="yele-label">Compare-at price</label>
                    <input
                      type="number"
                      step="0.01"
                      min="0"
                ...
                {/* Images */}
                <div>
                  <label className="yele-label">Images</label>
                  <p className="text-xs mb-2" style={{ color: '#8A8A92', fontFamily: 'var(--font-instrument)' }}>
                ...
                {/* Toggles */}
                <div>
                  <label className="yele-label">Track stock</label>
                  <Toggle value={form.track_inventory} onChange={(v) => patch({ track_inventory: v })} />
                </div>

                {form.track_inventory && (
                  <div>
                    <label className="yele-label">Stock</label>
                    <input
                      type="number"
                      min="0"
                      step="1"
                ...
                <div>
                  <label className="yele-label">Featured</label>
                  <Toggle value={form.featured} onChange={(v) => patch({ featured: v })} />
                </div>

                <div>
                  <label className="yele-label">Visible in store</label>
                  <Toggle value={form.active} onChange={(v) => patch({ active: v })} />
                </div>
              </div>

              <div className="p-5 flex gap-3 flex-shrink-0" style={{ borderTop: '1px solid rgba(22,22,26,0.06)' }}>
                <button
                  onClick={() => setPanelOpen(false)}
        ...
        {confirmDelete && (
          ...
          >
            <motion.div
              ...
              className="yele-card p-6 w-full max-w-sm"
            >
              <h4 className="font-semibold mb-2" style={{ color: '#16161A', fontFamily: 'var(--font-display)' }}>
                Delete this product?
              </h4>
              <p className="text-sm mb-5" style={{ color: '#8A8A92', fontFamily: 'var(--font-instrument)' }}>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  style={{ backgroundColor: '#B3382B', color: '#FFFFFF' }}
                  onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = '#8F2C22' }}
                  onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = '#B3382B' }}
                >
                  Delete
                </button>
              </div>
