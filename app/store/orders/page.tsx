...
const GREEN = { backgroundColor: 'rgba(6,95,70,0.08)', color: '#065f46' }
const BLUE = { backgroundColor: 'rgba(30,64,175,0.08)', color: '#1e40af' }
const RED = { backgroundColor: 'rgba(153,27,27,0.08)', color: '#991b1b' }
const GREY = { backgroundColor: 'rgba(0,0,0,0.06)', color: '#86868B' }
      ...
      <main className="flex-1 flex flex-col dashboard-main">
        ...
        <div className="flex-1 p-6 max-w-5xl space-y-6">
          ...
          {loading ? (
            ...
            >
              <p className="text-sm mb-1" style={{ ...S.body, color: '#1D1D1F' }}>
                Your account does not have an online store yet.
              </p>
              <p className="text-sm" style={{ ...S.body, color: '#86868B' }}>
            ...
            >
              ...
              >
                <div className="flex items-center gap-2">
                  <Receipt size={14} style={{ color: '#C8A97E' }} />
                  <h3
                ...
                <button
                  ...
                  style={{ ...S.body, color: showIncomplete ? '#1D1D1F' : '#86868B' }}
                >
              ...
              <div className="p-4">
                {ordersLoading ? (
                  ...
                  <div className="text-center py-10">
                    <p className="text-sm" style={{ ...S.body, color: '#1D1D1F' }}>No orders yet.</p>
                    <p className="text-sm mt-1" style={{ ...S.body, color: '#86868B' }}>
                        ...
                        <div className="flex items-center justify-between gap-3 flex-wrap">
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              <span style={{ ...S.pill, ...statusStyle(order.status) }}>
                                {statusLabel(order.status)}
                              </span>
                            </div>
                            <div className="text-xs mt-1 truncate" style={{ ...S.body, color: '#86868B' }}>
            ...
            >
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  <h4 className="font-semibold text-sm font-mono truncate" style={{ color: '#1D1D1F' }}>
                    {selected.order_number}
                  </h4>
                  <span style={{ ...S.pill, ...statusStyle(selected.status) }}>
              ...
              <div className="flex-1 p-5 space-y-6 overflow-y-auto">
                ...
                <div>
                  ...
                  <div className="text-sm" style={{ ...S.body, color: '#1D1D1F' }}>{selected.name || '—'}</div>
                  <div className="text-xs mt-0.5" style={{ ...S.body, color: '#86868B' }}>{selected.email || '—'}</div>
                  <div className="text-xs mt-0.5" style={{ ...S.body, color: '#86868B' }}>
                ...
                <div>
                  ...
                  {detailLoading ? (
                    <div className="flex flex-col gap-2 animate-pulse">
                      {[1, 2].map((i) => (
                        <div key={i} className="h-12 rounded-xl" style={{ backgroundColor: '#F5F5F7' }} />
                      ))}
                    </div>
                  ) : items.length === 0 ? (
                    <p className="text-sm" style={{ ...S.body, color: '#86868B' }}>No items on this order.</p>
                  ) : (
                    <div className="flex flex-col gap-2">
                      {items.map((item) => (
                        <div
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-sm truncate" style={{ ...S.body, color: '#1D1D1F' }}>{item.name}</div>
                            <div className="text-xs" style={{ ...S.body, color: '#86868B' }}>
                  ...
                  <div
                    ...
                    style={{ borderTop: '1px solid rgba(0,0,0,0.06)', ...S.body, color: '#1D1D1F' }}
                  >
                    <span className="font-semibold">Total</span>
                    <span className="font-semibold" style={{ color: '#C8A97E' }}>
                ...
                {/* Timeline */}
                <div>
                  <label style={S.label}>Customer timeline</label>
                  {detailLoading ? (
                    <div className="h-12 rounded-xl animate-pulse" style={{ backgroundColor: '#F5F5F7' }} />
                  ) : events.length === 0 ? (
                    <p className="text-sm" style={{ ...S.body, color: '#86868B' }}>
                      ...
                      {events.map((event) => (
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <span style={{ ...S.pill, ...statusStyle(event.status) }}>
                              {statusLabel(event.status)}
                            </span>
                            <span className="text-xs" style={{ ...S.body, color: '#86868B' }}>
                              {DATETIME_FMT.format(new Date(event.created_at))}
                            </span>
                          </div>
                          {event.message && (
                            <p className="text-xs mt-2 leading-relaxed" style={{ ...S.body, color: '#86868B' }}>
                              {event.message}
                            </p>
                          )}
                        </div>
                ...
                {/* Management block */}
                <div className="pt-5 space-y-4" style={{ borderTop: '1px solid rgba(0,0,0,0.06)' }}>
                  <div>
                  ...
                  <div>
                    <label style={S.label}>Message for the customer</label>
                    <textarea
                      value={messageDraft}
